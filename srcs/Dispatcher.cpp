...
// CGI module merged from WebServer-gabrielle (src/cgi.cpp)
static StatusCode executeCGI(HTTPRequest& request,
							 HTTPResponse& response) {
	...
	const std::string interpreter = location.interpreters.find(ext)->second;
	...
	// Read request body received in the temp upload file (POST)
	std::string input;
	if (!request.body.path.empty()) {
		...
		file.close();
		unlink(request.body.path.c_str()); // not an upload: clean up
	}
	...
	const std::string script_dir = path.substr(0, path.find_last_of('/') + 1);

	CGIResult cgi = run_cgi(interpreter, args, env, input, script_dir);

	if (cgi.exit_code != 0 && cgi.raw_output.empty()) {
		log.error("CGI execution failed: " + path);
		return INTERNAL_SERVER_ERROR;
	}

	response.setStatus(cgi.status != 0 ? static_cast<StatusCode>(cgi.status) : OK);
	response.setHeader("Connection", "keep-alive");

	std::string body_type = "text/html";
	...
	response.setBodySink(HEAP);
	response.setBody(cgi.body, body_type, request.headers_only);
...
StatusCode handleRequest(HTTPRequest& request,
	...
	if (hasCGIExtension(request)) {
		...
		if (request.resolved.method == POST &&
		   (request.body.size != 0 || request.body_chunked)) {
			// Body needed for CGI stdin: receive it first,
			// executeCGI runs on HTTPRequest::COMPLETE
			if (!createFile(request)) {
				return PAYLOAD_TOO_LARGE;
			}
			return NO_STATUS;
		}
		return executeCGI(request, response);
...
void Dispatcher::currentRequest(Client& client) {
	...
	case HTTPRequest::COMPLETE:
		if (hasCGIExtension(request)) {
			status_code = executeCGI(request, response);
		} else if (request.resolved.method == PUT) {
